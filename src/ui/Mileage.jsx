...
import EmployerList from './mileage/EmployerList.jsx';
import EmployerForm from './mileage/EmployerForm.jsx';
 ...
 *
 * The calculation runs per EMPLOYMENT × vehicle kind: each job carries its own
 * 10,000-mile allowance at the higher rate. Employers are optional — with none
 * recorded, every trip is one unnamed employment and the employer labels stay
 * out of the way.
 *
 * Only the trips and employers are stored. The band split, the running
 * position against the 10,000-mile line, and the claim are all computed at
 * read time by `gatherMileageData`, per the "never persist computed rows" rule.
 */
export default function Mileage() {
  ...
  const [confirmDeleteEmployer, setConfirmDeleteEmployer] = useState(null);
  ...
  const showEmployer = (data?.totals.employerCount ?? 0) > 1;
  ...
  const addEmployer = async (payload) => {
    await employersRepo.add(payload);
    setAddingEmployer(false);
  };
  const saveEmployer = async (payload) => {
    await employersRepo.update(editingEmployer.id, payload);
    setEditingEmployer(null);
  };
  const removeEmployer = async () => {
    await employersRepo.delete(confirmDeleteEmployer.id);
  ...
  return (
    <div className="screen">
      ...
      {addingEmployer && (
        <Modal title="Add employer" onClose={() => setAddingEmployer(false)}>
          <EmployerForm onSubmit={addEmployer} onCancel={() => setAddingEmployer(false)} />
        </Modal>
      )}
      {editingEmployer && (
        <Modal
          title={`Edit employer — ${editingEmployer.name}`}
          onClose={() => setEditingEmployer(null)}
        >
          <EmployerForm
            key={editingEmployer.id}
            initial={editingEmployer}
      ...
      {loading && !data ? (
        ...
        <>
          ...
          {/* Above the ledger, and shown even with no trips, so more than one
              job can be set up before the first journey is logged. */}
          <EmployerList
            employers={employers}
            onAdd={() => setAddingEmployer(true)}
            onEdit={setEditingEmployer}
            onDelete={setConfirmDeleteEmployer}
          />
      ...
      <ConfirmDialog
        open={!!confirmDeleteEmployer}
        title="Delete employer"
        message={
          confirmDeleteEmployer
            ? `Delete "${confirmDeleteEmployer.name}"? Its trips are kept but become unassigned, so they claim as one separate employment with its own 10,000-mile allowance.`
            : ''
        }
        confirmLabel="Delete"
        danger
        onConfirm={removeEmployer}
        onCancel={() => setConfirmDeleteEmployer(null)}
      />
    </div>
  );
}
